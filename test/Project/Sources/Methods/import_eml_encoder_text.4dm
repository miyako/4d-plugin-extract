...
$files:=Folder:C1567("/RESOURCES/eml").files(fk ignore invisible:K87:22 | fk recursive:K87:7)\
.query("extension == :1"; ".eml")
...
For each ($file; $files)
	...
	$extracted:=Extract(Extract Document EML; Extract Output Collection; $task)
