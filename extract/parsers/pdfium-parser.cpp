...
int pdf_get_page_text_size(FPDF_DOCUMENT doc, int page_index)
{
    FPDF_PAGE page = FPDF_LoadPage(doc, page_index);
    if (!page) return -1;

    FPDF_TEXTPAGE tp = FPDFText_LoadPage(page);
    if (!tp) { FPDF_ClosePage(page); return -1; }
    ...
    // measure UTF-8 size without allocating
    std::u16string utf16(nChars + 1, u'\0');
    FPDFText_GetText(tp, 0, nChars,
                     reinterpret_cast<unsigned short*>(utf16.data()));
    utf16.resize(nChars);
...
int pdf_get_page_text(FPDF_DOCUMENT doc, int page_index,
                      char* buf, int buf_len)
{
    ...
    // write UTF-8 directly into caller's buffer
    int written = 0;
    for (char16_t ch : utf16) {
        if (ch == 0) break;
        if (ch < 0x80) {
            if (written + 1 >= buf_len) break;
            buf[written++] = static_cast<char>(ch);
        } else if (ch < 0x800) {
            if (written + 2 >= buf_len) break;
            buf[written++] = 0xC0 | (ch >> 6);
            buf[written++] = 0x80 | (ch & 0x3F);
        } else {
            if (written + 3 >= buf_len) break;
            buf[written++] = 0xE0 | (ch >> 12);
            buf[written++] = 0x80 | ((ch >> 6) & 0x3F);
            buf[written++] = 0x80 | (ch & 0x3F);
        }
    }
    buf[written] = '\0'; // always null-terminate
